...
impl Dispatcher {
    ...
    pub async fn queue_function_by_name(
        ...
        return self
            ...
            .await
            .and_then(|result| Ok((result, recorder)));
    }
    ...
    async fn queue_function_sharded<'context>(
        ...
        non_caching: bool,
        recorder: Recorder,
    ) -> DandelionResult<(BTreeMap<usize, CompositionSet>, Vec<Recorder>)> {
        trace!(
        ...
        if let Some(mut result_set_map) = composition_set_maps.next() {
            ...
            return Ok((result_set_map, recorders));
        } else {
