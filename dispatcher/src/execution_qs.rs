...
use std::{
    cell::Cell, collections::{HashMap, VecDeque}, hint, sync::{Arc, Mutex}, time::{Duration, Instant, SystemTime, UNIX_EPOCH}
};

const MAX_QUEUE: usize = 4096;
const FRONT_QUEUE: usize = 10;
const MAX_QUEUE_TIME: Duration = Duration::new(0, 30_000_000); // 30 ms
const MAX_IDLE_TIME: Duration = Duration::new(0, 5_000_000); // 100 ms
...
impl WorkQueue for EngineQueueGPU {
    ...
    fn try_get_engine_args(&self) -> Option<(WorkToDo, Debt)> {
        ...
        if self
            ...
            .is_ok()
        {
            let function_queue_empty = { self.function_queue.lock().unwrap().is_empty() };
            if function_queue_empty {
                let try_result = self.general_queue_out.try_recv();
                self.worker_queue.start.fetch_add(1, Ordering::AcqRel);
                return match try_result {
                    Err(TryRecvError::Disconnected) => panic!("Work queue disconnected"),
                    Err(TryRecvError::Empty) => None,
    ...
    fn enqueue_work(&self, args: WorkToDo, function_id: FunctionId) -> DandelionResult<Promise> {
        ...
        match args {
            WorkToDo::FunctionArguments { .. } => {
                let now = Instant::now();
                self.function_queue.lock().unwrap().push_back((now, function_id, args, debt));
            },
            _ => {
                match self.general_queue_in.try_send((args, debt)) {
                    Ok(()) => (),
                    Err(TrySendError::Disconnected(_)) => {
                        error!("Failed to enqueu work, workqueue has been disconnected")
                    }
