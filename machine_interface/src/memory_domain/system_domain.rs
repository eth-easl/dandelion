...
impl ContextTrait for SystemContext {
    ...
    fn read<T>(&self, offset: usize, read_buffer: &mut [T]) -> DandelionResult<()> {
        ...
        while total_bytes_read < read_buffer_size{
            ...
            let mut range = self.local_offset_to_data_position.range((Unbounded, Included(offset + total_bytes_read)));
                ...
                if (offset + total_bytes_read) < item_offset + item_size {
                    ...
                    match item_position{
                        ...
                        DataPosition::ResponseInformationStorage(ref body) => {
                            ...
                            let read_memory = unsafe {core::slice::from_raw_parts_mut(read_buffer.as_mut_ptr() as *mut u8, read_buffer_size)};
    ...
    fn get_chunk_ref(&self, offset: usize, length: usize) -> DandelionResult<&[u8]> {
        ...
        let mut range = self.local_offset_to_data_position.range((Unbounded, Included(offset)));
        if let Some((&item_offset, &(ref item_position, item_size))) = range.next_back() {
...
pub fn system_context_write_from_bytes(
    ...
    assert_eq!(size, source.len(), "size is not equal to source size (system_context_write_from_bytes)");
    ...
    match &source.context{
        ContextType::System(source_ctxt) => {
            ...
            while transfered_bytes < size{
                ...
                let Some((data_position, item_size)) = source_ctxt.local_offset_to_data_position.get(&(source_offset + transfered_bytes))
                else {
                    warn!("Read offset not stored in SystemContext (system_context_transfer). Offset: {}", source_offset + transfered_bytes);
                    return Err(DandelionError::InvalidRead);
                };

                if transfered_bytes + item_size > size {
                    warn!("Memory transfer would not involve whole item! (system_context_transfer). 
                ...
                match data_position{
                    DataPosition::ContextStorage(data_arc_context, actual_offset) => 
                        {destination.local_offset_to_data_position.insert(destination_offset + transfered_bytes, (DataPosition::ContextStorage(data_arc_context.clone(), *actual_offset), *item_size));}
                    DataPosition::ResponseInformationStorage(ref body) => 
                        {destination.local_offset_to_data_position.insert(destination_offset + transfered_bytes, (DataPosition::ResponseInformationStorage(body.clone()), *item_size));}
                    _ =>
    ...
    return match &source.context{
        ContextType::System(source_ctxt) => {
            ...
            while transfered_bytes < size{
                ...
                if transfered_bytes + item_size > size {
                    warn!("Memory transfer would not involve whole item! (out_of_system_context_transfer). 
                        destination_offset: {}, transfered_bytes: {}, total_size: {}, item_size: {}", 
                        destination_offset, transfered_bytes, size, item_size);
                    return Err(DandelionError::InvalidRead);
                }
                ...
                match data_position {
                    DataPosition::ContextStorage(data_arc_context, actual_offset) => {
                        transfer_memory(destination, data_arc_context.clone(), destination_offset + transfered_bytes, *actual_offset, *item_size)?;
                    }
                    DataPosition::ResponseInformationStorage(ref body) => {
