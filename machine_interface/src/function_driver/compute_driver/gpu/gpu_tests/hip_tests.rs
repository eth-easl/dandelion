...
use crate::{
    function_driver::{
        compute_driver::{
            ...
            gpu::gpu_tests::{get_driver, Arc, Archive, ArchiveInit, RecordPoint, GPU_LOCK},
        },
...
fn minimal() {
    ...
    engine_minimal::<GpuMemoryDomain>(
        &format!(
            "{}/tests/data/hip/test_gpu_minimal.json",
...
fn basic_input_output() {
    ...
    let (mut function_context, config, queue) = prepare_engine_and_function::<GpuMemoryDomain>(
        &format!(
            "{}/tests/data/hip/test_gpu_basic_io.json",
            env!("CARGO_MANIFEST_DIR")
        ),
        MemoryResource::None,
        &driver,
...
fn engine_matmul_3x3_loop() {
    let lock = GPU_LOCK.lock().unwrap();
    let filename = &format!(
        "{}/tests/data/hip/test_gpu_matmul_loop.json",
        env!("CARGO_MANIFEST_DIR")
    );
...
fn engine_matmul_size_sweep_parallel() {
    let lock = GPU_LOCK.lock().unwrap();
    let filename = &format!(
        "{}/tests/data/hip/test_gpu_matmul_para.json",
        env!("CARGO_MANIFEST_DIR")
    );
    let dom_init = MemoryResource::None;
    let driver: Box<dyn Driver> = get_driver();
...
fn inference_benchmark_function() {
    let lock = GPU_LOCK.lock().unwrap();
    let filename = &format!(
        "{}/tests/data/hip/test_gpu_inference.json",
        env!("CARGO_MANIFEST_DIR")
    );
    let dom_init = MemoryResource::None;
    let driver: Box<dyn Driver> = get_driver();
...
fn hello_world() {
    ...
    let (function_context, config, queue) = prepare_engine_and_function::<GpuMemoryDomain>(
        &format!(
            "{}/tests/data/hip/test_gpu_hello_world.json",
            env!("CARGO_MANIFEST_DIR")
        ),
        MemoryResource::None,
        &driver,
...
#[test]
fn engine_first_double_matmul() {
    use std::env;
    env::set_var("RUST_BACKTRACE", "1");

    let lock = GPU_LOCK.lock().unwrap();
    let filename = &format!(
        "{}/tests/data/hip/test_gpu_first_double_matmul.json",
        env!("CARGO_MANIFEST_DIR")
    );
    let dom_init = MemoryResource::None;
    let driver: Box<dyn Driver> = get_driver();
