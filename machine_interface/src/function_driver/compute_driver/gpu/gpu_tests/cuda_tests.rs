...
fn minimal() {
    ...
    engine_minimal::<GpuMemoryDomain>(
        &format!(
            "{}/tests/data/cuda/test_gpu_minimal.json",
...
fn minimal_matmul() {
    let lock = GPU_LOCK.lock().unwrap();
    let filename = &format!(
        "{}/tests/data/cuda/test_gpu_matmul.json",
        env!("CARGO_MANIFEST_DIR")
    );
    ...
    let expected = vec![
        112f32, 124f32, 136f32, 148f32, 304f32, 348f32, 392f32, 436f32, 496f32, 572f32, 648f32,
        724f32, 688f32, 796f32, 904f32, 1012f32,
    ];
