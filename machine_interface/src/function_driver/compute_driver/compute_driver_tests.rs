use crate::{
    ...
    function_driver::{
        functions::FunctionAlternative, test_queue::TestQueue, ComputeResource, Metadata, WorkToDo,
    },
...
fn driver(engine_type: EngineType, init: Vec<ComputeResource>, wrong_init: Vec<ComputeResource>) {
...
fn engine_matmul_size_sweep<Dom: MemoryDomain>(
    ...
    for mat_size in LOWER_SIZE_BOUND..UPPER_SIZE_BOUND {
        let (domain, queue) =
            prepare_engine_and_domain::<Dom>(dom_init, engine_type, drv_init.clone());
        // add inputs
        ...
        let mut input_context = ReadOnlyContext::from_boxed(mat_vec.into_boxed_slice()).unwrap();
...
macro_rules! driverTests {
    ($name : ident; $domain : ty; $dom_init: expr; $engine_type : expr ; $drv_init : expr; $drv_init_wrong : expr) => {
        ...
        fn test_engine_matmul_size_sweep() {
            ...
            super::engine_matmul_size_sweep::<$domain>(&name, $dom_init, $engine_type, $drv_init);
        }
