...
pub enum DandelionError {
    ...
    /// errors related to the set registry in the dispatcher
...
pub enum SetRegistryError {
    /// Attempted to register a new set that already existed
    InsertExisting,
    /// Attempt to access a set to await / contribute to, that was not registered
    NonExisting,
    /// Cannot get actual results from registry (e.g. producer timed out)
    UnretrievableSet,
}
