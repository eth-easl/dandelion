...
fn main() -> () {
    ...
    let (dispatcher, resource_pool, cpu_core_map) = Box::leak(Box::new(
        Dispatcher::init(resource_pool, cpu_core_map, memory_pool, threads_per_core, cpu_pinning, compute_range).expect("Should be able to start dispatcher"),
    ));
    // start dispatcher
