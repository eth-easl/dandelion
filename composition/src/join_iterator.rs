...
pub(super) trait JoinIterator {
    ...
    fn fill_in(&mut self, to_fill: &mut Vec<DataSet>);

    /// Advances the iterator by one.
    /// An `advance` call following an `advance` that returned `false` always returns `false`.
    /// A `fill_in` call after an `advance` that returned `false` is undefined behaviour.
...
impl SetAllIterator {
    pub(super) fn new(
        ...
        debug_assert_ne!(0, set.items.len());
        Some(Box::new(Self {
            left,
            set,
            write_idx,
...
impl JoinIterator for SetAllIterator {
    ...
    fn fill_in(&mut self, to_fill: &mut Vec<DataSet>) {
        to_fill[self.write_idx] = self.set.clone();
...
impl SetKeyIterator {
    ...
    pub(super) fn new(
        ...
        let mut start_idx = 0;
        for chunk in set.items.chunk_by(|a, b| a.key == b.key) {
            let key = chunk[0].key;
...
impl JoinIterator for SetKeyIterator {
    ...
    fn fill_in(&mut self, to_fill: &mut Vec<DataSet>) {
        ...
        if fill_this_set {
            let item_list = self.items[self.key_groups[self.key_groups_idx].1.clone()].to_vec();
...
impl AnyIterator {
    ...
    pub(super) fn new(
        ...
        if let Some(mut it) = inner_join_it {
            ...
            first_sets.resize(num_sets, DataSet::default());
            it.fill_in(&mut first_sets);
            for set in first_sets.iter() {
                total_sizes.push(set.total_size);
            }
            inner_sharding.push(first_sets);
            while it.advance() {
                let mut next_sets = Vec::with_capacity(num_sets);
                next_sets.resize(num_sets, DataSet::default());
                it.fill_in(&mut next_sets);
                for (i, set) in next_sets.iter().enumerate() {
                    total_sizes[i] += set.total_size;
                }
            ...
            if self.min_set_bytes > 0 {
                self.set_groups.sort_by(|a, b| {
                    a[self.largest_set_idx]
                        .total_size
                        .cmp(&b[self.largest_set_idx].total_size)
                });
                ...
                for group_idx in 0..parallelism {
                    ...
                    set_group.resize(num_sets, DataSet::default());
                    ...
                    {
                        let mut accum = DataSetAccumulator::new();
                        let mut curr_set_size = 0;
                        while curr_idx < self.set_groups.len()
                            && (curr_idx < min_end_idx || curr_set_size < self.min_set_bytes)
                        {
                            curr_set_size +=
                                self.set_groups[curr_idx][self.largest_set_idx].total_size;
                            accum.push_set(mem::take(
                                &mut self.set_groups[curr_idx][self.largest_set_idx],
                            ));
                            curr_idx += 1;
                        }
                        set_group[self.largest_set_idx] = accum.collect();
                    }
                    ...
                    for set_idx in 0..num_sets {
                        ...
                        for i in start_idx..curr_idx {
                            accum.push_set(mem::take(&mut self.set_groups[i][set_idx]));
                        }
                        set_group[set_idx] = accum.collect();
                    }
